...
app.get('/instruments/keyboard', async (req, res, next) => {
    const keyboards = await Instrument.scope(['defaultScope', 'keyboards']).findAll()
    res.json(keyboards);
});


app.get('/instruments/string', async (req, res, next) => {
    const strings = await Instrument.scope(['defaultScope', 'strings']).findAll()
    res.json(strings);
});

app.get('/instruments/woodwind', async (req, res, next) => {
    const woodWinds = await Instrument.scope(['defaultScope', 'woodwinds']).findAll()
    res.json(woodWinds);
});
...
app.get('/stores/:storeId/instruments', async (req, res, next) => {
    // const filterStoreInstruments = await Instrument.scope([{method: ['atStore', req.params.storeId]}]).findAll()
    const store = await Store.scope([{method: ['inStore']}]).findByPk(req.params.storeId)
    // Your code here
    res.json(store);
});
